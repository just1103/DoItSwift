...
class ViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    let MAX_ARRAY_NUM = 10 // 왜 갑자기 대문자로 선언하지? - 이미지의 파일명을 저장할 배열의 최대 크기
 ...
 // 피커뷰의 동작에 필요한 델리게이트 메서드 3개 선언 - column 개수, row 개수, 옵션1)row title, 옵션2)row image를 지정 (ViewController Class에 선언되어있는 메서드임)
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return imageFileName.count // 이게 row로 자동입력 된건가?
...
//    // 옵션1) returns the title of each row (이게 없으면 피커뷰의 row가 모두 "?"로 뜸)
//    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
//        return imageFileName[row] // Array[row] <- 어떻게 row에 자동으로 0...9가 입력됐지? (titleForRow row: Int -> 여기서 룰렛의 column&row 값을 가져오는 듯??)
